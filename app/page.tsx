      ...
      <section className="border-t border-white/10 bg-white/[0.02] py-20 md:py-28">
        <Container>
          <SectionHeading
            ...
            title="Pricing"
            description="Pick the IPTV subscription that fits your home. No contract and no hidden fees. Every plan includes all channels, all devices and 24/7 support."
          />

          <div className="mt-14 grid gap-4 lg:grid-cols-3">
            {plans.map((plan, i) => {
              const badge = ["Save 15%", "Best value", "Save 32%"][i];
              return (
                <div
                  key={plan.id}
                ...
                >
                  {plan.featured ? (
                    <span className="absolute -top-3 left-6 rounded-full bg-gradient-to-r from-brand-1 to-brand-3 px-3 py-1 text-[11px] font-semibold text-[#1a0f07]">
                      {badge}
                    </span>
                  ) : null}
                  ...
                  {!plan.featured ? (
                    <p className="mt-1 text-xs text-white/40">{badge}</p>
                  ) : null}
                  ...
                  <p className="mt-3 text-center text-xs text-white/40">
                    {plan.connections}{" "}
                    {plan.connections === 1 ? "connection" : "connections"} · no
                    contract
                  </p>
                </div>
              );
